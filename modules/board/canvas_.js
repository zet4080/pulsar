...
const iterateTiles = function (board, fnction) {
...
const drawGameElements = function (board) {
    ...
    table.save();            
    table.scale(viewport.scale, viewport.scale);
    iterateTiles(board, function (tile, position) {
        table.save();
        table.drawImage(tile.image, tile.sx, tile.sy, tile.swidth, tile.sheight, position.x, position.y, position.width, position.height);
        table.restore();
    });
    table.restore();        
};
